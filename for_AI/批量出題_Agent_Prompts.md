...
```text
你是 K-12 與大專程式教育課程設計師。請為 PyJudge 規劃一組可自動判分的程式題，但此階段不要寫完整題目與程式碼。
...
```text
你是資深 Online Judge 出題者、兒童程式教育教師與測試工程師。請依核准的題組藍圖，在目前 PyJudge repository 中產生可直接上線的題目。
...
```text
你是獨立的 Online Judge 審題員、兒童發展適齡審查員與 adversarial tester。請審查目前 git diff 中新增的 PyJudge 題目；不要相信生成者的自我檢查。
...
```text
請在目前 PyJudge repository 中批量設計、實作並驗證 {{COUNT}} 題程式題。
