...
2. 指定學習階段（`stage`），不得只按年齡或單一數學知識判斷：
   - `Beginner`（初階）：國小、程式初學者或尚未熟悉操作；一題聚焦一個核心概念，包含基本輸入輸出、變數、四則運算、單層條件與單層迴圈，原則上可用 Scratch／Blockly 完成。
   - `Intermediate`（中階）：已掌握基礎語法，能組合 2～3 個概念；可能使用國中數學，並涵蓋多分支、巢狀迴圈、字串、清單、座標、簡單模擬與基礎數論。不能因只用到小學數學就自動降為初階。
   - `Advanced`（高階）：國中資優、高中及以上；需要選擇演算法、理解時間複雜度或進行較長的狀態推理，例如搜尋、排序、遞迴、二維資料、貪心與基礎動態規劃。
...
分級時依序檢查：需要組合多少概念、是否必須選擇演算法、資料範圍是否淘汰暴力法、實作是否容易出錯，最後才參考數學年級。核心原則為：**初階學語法，中階組合概念，高階選擇演算法，挑戰處理效率與競賽限制。**
